...
\captionsetup{justification=centering,singlelinecheck=false,format=hang,font={footnotesize,bf},labelfont=bf}

\begin{comment}
\newlistof{lstlistoflistings}{lol}{\lstlistlistingname}
\newlistentry{listing}{lol}{0}%% Entrada da lista de objetos flutuantes (algoritmo)
\newfloat{listing}{lol}{\listingname}%% Objeto flutuante (algoritmo)
\setfloatadjustment{listing}{\centering}%% Ajuste do objeto flutuante (algoritmo)
...
\let\oldlistoflistings\listoflistings \renewcommand{\listoflistings}{%
 \begingroup%
 \let\oldnumberline\numberline%
 \renewcommand{\numberline}{\lstlistingname~\oldnumberline}%
 \oldlistoflistings*%
...
\def\lst@MakeCaption#1{%
    ...
    \ifx b#1%
        \ifx\lst@source\@empty\else
            \noindent\vskip\belowcaptionskip
...
%\renewcommand*{\cftfigaftersnum}{~~--~~}%% Conteúdo após a entrada na lista de objetos flutuantes (algoritmo)
%\renewcommand{\afterloltitle}{\afterchaptertitle\vspace*{0.5\linha}}%% Espaçamento após o título da lista (algoritmo)
%\setfloatadjustment{lstlisting}{\centering}%% Ajuste do objeto flutuante (algoritmo)
