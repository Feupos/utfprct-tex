...
#ifdef LIBNOP_BENCHMARK_SEMAPHORE_ENABLE
static void BM_SemaphoreCTA(benchmark::State& state)
{
    for (auto _ : state)
    {
        std::array<SemaphoreCTA, 100> semaphores;
        for (int i = 0; i < 4000; i++)
        {
            std::for_each(std::execution::par_unseq, semaphores.begin(),
                          semaphores.end(), [&](auto& semaphore) {
                              semaphore.atSeconds->SetValue<NOP::Parallel>(
                                  semaphore.atSeconds->GetValue() + 1);
                          });
        }
    }
}

BENCHMARK(BM_SemaphoreCTA)->MinTime(1);

static void BM_SemaphoreCBCL(benchmark::State& state)
{
    for (auto _ : state)
    {
        std::array<Semaphore_CBCL, 100> semaphores;
        for (int i = 0; i < 4000; i++)
        {
            std::for_each(std::execution::par_unseq, semaphores.begin(),
                          semaphores.end(), [&](auto& semaphore) {
...
BENCHMARK(BM_SemaphoreCBCL)->MinTime(1);
#endif
